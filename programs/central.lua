-- /programs/central.lua  v13
-- Fix Fase 8.9:
--  - Padding tra sezioni per dare piu' aria
--  - Tutti i pulsanti API (nav DETAIL, +/- PROJECT) in basso vicino ai tab buttons
--  - Header colonne dei tab spostato a riga 14 (in basso)
...
print("=== CENTRAL DASHBOARD v13 ===")
logger.info("Central v13 avviato")
...
local function renderTop()
    ...
    -- HEADER (riga 3, dopo padding riga 2)
    ...
    if es.ok then
        ...
        p_top:text(3, 5, "OFFLINE", P.fg_crit, P.bg)
    end

    -- riga 7: padding (vuoto)
    ...
    p_top:text(3, 13, satLabel, satCol, P.bg)
    if es.ok then
        local avg = es.avg_flow or 0
        local acol = (avg >= 0) and P.fg_ok or P.fg_crit
        local avgStr = "AVG:" .. ((avg >= 0) and "+" or "") .. fmt(avg)
        p_top:text(math.floor(W / 2) - math.floor(#avgStr / 2), 13, avgStr, acol, P.bg)
    end
    local upStr = "UP:" .. fmtUptime()
    p_top:text(W - #upStr - 2, 13, upStr, P.fg_dim, P.bg)
...
-- 3: header
-- 4: div STATUS
-- 5-7: status (3 righe: stored, capacity, level)
-- 8: padding
-- 9: FLOW + AVG + ST inline
-- 10: div POWER LEVEL HIST 30M
-- 11-12: chart power 2 righe
-- 13: I/O+ verde
-- 14: 0 zero line
-- 15: I/O- rosso
-- 16: bordo
...
local function renderEnergyDetail()
    ...
    -- riga 9: FLOW + AVG + STATUS inline
    local flow = s.flow or 0
    local fcol = (flow >= 0) and P.fg_ok or P.fg_crit
    local avg = s.avg_flow or 0
    local acol = (avg >= 0) and P.fg_ok or P.fg_crit
    p_lat3:text(3, 9, "FLOW:", P.fg_label, P.bg)
    p_lat3:text(8, 9, ((flow >= 0) and "+" or "") .. fmt(flow), fcol, P.bg)
    p_lat3:text(16, 9, "AVG:", P.fg_label, P.bg)
    p_lat3:text(20, 9, ((avg >= 0) and "+" or "") .. fmt(avg), acol, P.bg)
    local statusLabel
    if math.abs(flow) < 1 then       statusLabel = "STABLE"
    elseif flow > 0 then             statusLabel = "CHARG"
    else                             statusLabel = "DISCH" end
    local statCol = (flow > 0) and P.fg_ok or (flow < 0) and P.fg_warn or P.fg_dim
    p_lat3:text(W - 9, 9, "ST:" .. statusLabel, statCol, P.bg)

    -- POWER LEVEL HISTORY (10-12)
    ...
    -- I/O FLOW chart sym 3 righe (13-15)
    ...
    p_lat3:text(2, 13, "+" .. fmt(maxAbs), P.fg_ok,   P.bg)
    p_lat3:text(2, 14, "  0",                P.fg_dim,  P.bg)
    p_lat3:text(2, 15, "-" .. fmt(maxAbs), P.fg_crit, P.bg)
    if #flowHist >= 2 then
        p_lat3:sparklineSym(chartX, 14, chartW, 1, flowHist,
            { posColor = P.fg_ok, negColor = P.fg_crit, maxAbs = maxAbs })
    else
        p_lat3:textCenter(14, "(in raccolta...)", P.fg_dim, P.bg)
    end
...
-- 1: bordo, 2: padding
-- 3: header APIARY MONITOR  time
-- 4: div con titolo tab corrente
-- 5-13: tab content (9 righe)
-- 14: nav buttons (DETAIL: < PREV / NEXT >; PROJECT: IN [X]h [-][+])
--     o header promemoria colonne (LIST/CHARTS)
-- 15: tab buttons (DETAIL/LIST/PROJ/CHRT)
-- 16: bordo
-- ============================================================
...
local function renderApiTabDettaglio()
    ...
    p_lat4:text(3, 9,  "EMA   :", P.fg_label, P.bg)
    p_lat4:text(13, 9, fmtRate(s.rate_ema), (s.rate_ema >= 0) and P.fg_ok or P.fg_crit, P.bg)
    ...
    p_lat4:text(13, 10, arrow .. " " .. tlbl, acol, P.bg)
...
local function renderApiTabProiezione()
    ...
    for i, prod in ipairs(bees.getProducts()) do
        ...
        p_lat4:text(3,  row, shorten(prod.label, 14), P.fg_value, P.bg)
        p_lat4:text(20, row, fmt(fut), P.fg_value, P.bg)
        p_lat4:text(30, row, ((d >= 0) and "+" or "") .. fmt(d), dcol, P.bg)
    end
end
...
local function renderApi()
    ...
    -- Divider con titolo tab corrente (riga 4)
    ...
    local tabTitle = (state.api_tab == "dettaglio" and " DETAIL ")
                  ...
                  or " CHARTS "
    p_lat4:text(3, 4, tabTitle, P.fg_title, P.bg)
    ...
    -- Riga 14: pulsanti contestuali al tab attuale (NAV / CONTROLS / HEADER colonne)
    if state.api_tab == "dettaglio" then
        -- Bottoni navigazione PREV/NEXT
        p_lat4:button("api_prev", 3,        14, 8, 1, " < PREV", colors.blue, colors.white)
        p_lat4:button("api_next", W - 10,   14, 8, 1, "NEXT > ", colors.blue, colors.white)
    elseif state.api_tab == "proiezione" then
        -- Controls IN [X]h [-] [+]
        p_lat4:text(3, 14, "IN:", P.fg_label, P.bg)
        p_lat4:fill(7, 14, 4, 1, colors.gray)
        p_lat4:text(8, 14, ("%3d"):format(state.api_proj_hours), P.fg_value, colors.gray)
        p_lat4:text(12, 14, "h", P.fg_label, P.bg)
        p_lat4:button("api_h_minus", 15, 14, 4, 1, " - ", colors.red,   colors.white)
        p_lat4:button("api_h_plus",  21, 14, 4, 1, " + ", colors.green, colors.white)
        p_lat4:text(28, 14, "+" .. state.api_proj_hours .. "h DELTA", P.fg_dim, P.bg)
    elseif state.api_tab == "lista" then
        -- Header colonne come promemoria
        p_lat4:text(3,  14, "PRODUCT", P.fg_dim, P.bg)
        p_lat4:text(18, 14, "STORED", P.fg_dim, P.bg)
        p_lat4:text(27, 14, "RATE",   P.fg_dim, P.bg)
        p_lat4:text(36, 14, "TR",     P.fg_dim, P.bg)
    elseif state.api_tab == "grafici" then
        p_lat4:text(3,  14, "PRODUCT",   P.fg_dim, P.bg)
        p_lat4:text(15, 14, "RATE",      P.fg_dim, P.bg)
        p_lat4:text(24, 14, "CHART 30M", P.fg_dim, P.bg)
    end
...
print("")
logger.info("Task paralleli avviati (v13)")
