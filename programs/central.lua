-- /programs/central.lua  v10
-- Scale 1.5 sui monitor principali (testo grande, ben leggibile da lontano)
-- Compromesso: meno informazioni visibili contemporaneamente.
...
print("=== CENTRAL DASHBOARD v10 ===")
logger.info("Central v10 avviato")
...
-- SCALE 1.5 sui monitor con dati testuali, scale 1 sulle barre verticali
p_top:setScale(1.5)
p_bar1:setScale(1)
p_bar2:setScale(1)
p_lat3:setScale(1.5)
...
-- ============================================================
-- Render TOP a scale 1.5 (~32x16)
-- 1: bordo, 2: padding, 3: header, 4: div
-- 5-7: ENERGY, 8: div
-- 9-13: STORAGE, 14: div
-- 15: SAT info + bottoni FARM (1 riga)
-- 16: bordo
-- ============================================================
local function renderTop()
    ...
    -- HEADER (riga 3)
    p_top:text(3, 3, "CENTRALE", P.fg_title, P.bg)
    local overall = health.overall(alerts.timeouts)
    local hLabel, hCol
    if overall == "ok" then
        hLabel = "[OK]"; hCol = P.fg_ok
    elseif overall == "degraded" then
        hLabel = "[DEGRAD]"; hCol = P.fg_warn
    else
        hLabel = "[CRIT]"
        hCol = state.blink_on and P.fg_crit or P.bg
    end
    p_top:text(13, 3, hLabel, hCol, P.bg)
    local time_str = textutils.formatTime(os.time("local"), true)
    p_top:text(W - #time_str - 2, 3, time_str, P.fg_dim, P.bg)

    -- ENERGY (4-7)
    p_top:divider(1, 4, W, P.border)
    p_top:text(3, 4, " ENERGY ", P.fg_title, P.bg)
    ...
    if es.ok then
        ...
        p_top:text(W - #pctStr - 2, 4, pctStr, ecol, P.bg)

        p_top:text(3, 5, fmt(es.stored) .. " / " .. fmt(es.capacity) .. " FE", P.fg_value, P.bg)

        local flow = es.flow or 0
        local fcol = (flow >= 0) and P.fg_ok or P.fg_crit
        p_top:text(3, 6, "FLOW: " .. ((flow >= 0) and "+" or "") .. fmt(flow) .. " FE/t", fcol, P.bg)

        -- Mini sparkline 1 riga
        local hist = energy.getPctHistory(1800)
        if #hist >= 2 then
            p_top:sparklineMulti(3, 7, W - 4, 1, hist, { color = ecol, min = 0, max = 1 })
        end
    else
        p_top:text(3, 5, "OFFLINE - retry", P.fg_crit, P.bg)
    end

    -- STORAGE (8-13)
    p_top:divider(1, 8, W, P.border)
    p_top:text(3, 8, " STORAGE ", P.fg_title, P.bg)

    local rss = rs.snapshot()
    health.set("rs", rss.ok)
    if rss.ok then
        local pctStr = ("%.3f%%"):format(rss.item_pct * 100)
        p_top:text(W - #pctStr - 2, 8, pctStr, colors.cyan, P.bg)

        p_top:text(3, 9, fmt(rss.item_used) .. " / " .. fmt(rss.item_max) .. " items", P.fg_value, P.bg)

        -- Composition: 4 item compatti su 2 righe (2 per riga)
        if state.storage_breakdown and #state.storage_breakdown.breakdown > 0 then
            local items = state.storage_breakdown.breakdown
            -- riga 10: top 1 e top 2
            for col = 1, 2 do
                if items[col] then
                    local b = items[col]
                    local x = 3 + (col - 1) * (math.floor(W / 2) - 1)
                    local segCol = STACK_COLORS[col] or colors.gray
                    p_top:fill(x, 10, 2, 1, segCol)
                    local nm = shorten(shortName(b.name), 6)
                    local pcs = ("%2d%%"):format(math.floor(b.pct * 100 + 0.5))
                    p_top:text(x + 3, 10, nm .. " " .. pcs, P.fg_value, P.bg)
                end
            end
            -- riga 11: top 3 e top 4
            for col = 1, 2 do
                local idx = col + 2
                if items[idx] then
                    local b = items[idx]
                    ...
                    p_top:fill(x, 11, 2, 1, segCol)
                    local nm = shorten(shortName(b.name), 6)
                    local pcs = ("%2d%%"):format(math.floor(b.pct * 100 + 0.5))
                    p_top:text(x + 3, 11, nm .. " " .. pcs, P.fg_value, P.bg)
                end
            end
            -- OTHERS riga 12
            if state.storage_breakdown.others_pct > 0 then
                p_top:fill(3, 12, 2, 1, OTHERS_COLOR)
                local pcs = ("%d%%"):format(math.floor(state.storage_breakdown.others_pct * 100 + 0.5))
                p_top:text(6, 12, "OTHERS " .. pcs, P.fg_dim, P.bg)
            end
        end
    else
        p_top:text(3, 9, "OFFLINE", P.fg_crit, P.bg)
    end

    -- FOOTER (13-15)
    p_top:divider(1, 13, W, P.border)

    -- Riga 14: satellite info + uptime
    ...
    p_top:text(3, 14, satLabel, satCol, P.bg)
    local upStr = "UP:" .. fmtUptime()
    p_top:text(W - #upStr - 2, 14, upStr, P.fg_dim, P.bg)
    if es.ok then
        local avg = es.avg_flow or 0
        local acol = (avg >= 0) and P.fg_ok or P.fg_crit
        local avgStr = "AVG:" .. ((avg >= 0) and "+" or "") .. fmt(avg)
        local cx = math.floor((W - #avgStr) / 2) + 1
        p_top:text(cx, 14, avgStr, acol, P.bg)
    end

    -- Bottoni FARM (riga 15) - 2 bottoni affiancati
    local btnW = math.floor((W - 6) / 2)
    local btn1X = 3
    local btn2X = 3 + btnW + 2

    local f1On = state.farms[1] == true
    local f1Bg = f1On and colors.green or colors.red
    local f1Lbl = f1On and "FARM1: ON" or "FARM1: OFF"
    p_top:button("farm1", btn1X, 15, btnW, 1, f1Lbl, f1Bg, colors.white)

    local f2On = state.farms[2] == true
    local f2Bg = f2On and colors.green or colors.red
    local f2Lbl = f2On and "FARM2: ON" or "FARM2: OFF"
    p_top:button("farm2", btn2X, 15, btnW, 1, f2Lbl, f2Bg, colors.white)

    p_top:flush()
end

-- ============================================================
-- BAR1: STORAGE (scale 1)
-- ============================================================
...
local function renderEnergyDetail()
    ...
    p_lat3:text(3, 3, "ENERGY DETAIL", P.fg_title, P.bg)
    p_lat3:text(W - 11, 3, "VIBRANT CB", P.fg_dim, P.bg)
    ...
    -- STATUS (4-9)
    p_lat3:divider(1, 4, W, P.border)
    p_lat3:text(3, 4, " STATUS ", P.fg_title, P.bg)

    p_lat3:text(3,  5, "STORED  :", P.fg_label, P.bg)
    p_lat3:text(13, 5, fmt(s.stored) .. " FE", P.fg_value, P.bg)

    p_lat3:text(3,  6, "CAPACITY:", P.fg_label, P.bg)
    p_lat3:text(13, 6, fmt(s.capacity) .. " FE", P.fg_value, P.bg)

    p_lat3:text(3, 7, "LEVEL   :", P.fg_label, P.bg)
    local ecol = energyColorAlert(s.pct)
    local barW = W - 24
    p_lat3:hbar(13, 7, barW, s.pct, ecol, colors.gray)
    p_lat3:text(W - 8, 7, ("%5.1f%%"):format(s.pct * 100), ecol, P.bg)

    p_lat3:text(3, 8, "FLOW    :", P.fg_label, P.bg)
    local flow = s.flow or 0
    local fcol = (flow >= 0) and P.fg_ok or P.fg_crit
    p_lat3:text(13, 8, ((flow >= 0) and "+" or "") .. fmt(flow) .. " FE/t", fcol, P.bg)
    -- Status inline
    local statusLabel
    ...
    p_lat3:text(W - 14, 8, "ST:" .. statusLabel, statCol, P.bg)

    p_lat3:text(3, 9, "AVG 10s :", P.fg_label, P.bg)
    local avg = s.avg_flow or 0
    local acol = (avg >= 0) and P.fg_ok or P.fg_crit
    p_lat3:text(13, 9, ((avg >= 0) and "+" or "") .. fmt(avg) .. " FE/t", acol, P.bg)

    -- POWER LEVEL HISTORY (10-15)
    p_lat3:divider(1, 10, W, P.border)
    p_lat3:text(3, 10, " POWER LEVEL HIST 30M ", P.fg_title, P.bg)
    ...
    p_lat3:text(2, 11, "100", P.fg_dim, P.bg)
    p_lat3:text(2, 13, "  0", P.fg_dim, P.bg)

    if #hist30 >= 2 then
        p_lat3:sparklineMulti(chartX, 11, chartW, chartH, hist30,
            { color = ecol, min = 0, max = 1 })
    else
        p_lat3:textCenter(12, "(in raccolta...)", P.fg_dim, P.bg)
    end

    p_lat3:text(chartX, 14, "-30m", P.fg_dim, P.bg)
    p_lat3:text(chartX + math.floor(chartW / 2) - 2, 14, "-15m", P.fg_dim, P.bg)
    p_lat3:text(chartX + chartW - 4, 14, " now", P.fg_dim, P.bg)

    -- I/O FLOW compatto (15)
    local flowHist = energy.getFlowHistory(300)
    if #flowHist >= 2 then
        p_lat3:text(3, 15, "I/O 10M:", P.fg_dim, P.bg)
        local maxAbs = 1
        for _, v in ipairs(flowHist) do
            if math.abs(v) > maxAbs then maxAbs = math.abs(v) end
        end
        p_lat3:sparklineMulti(12, 15, W - 14, 1, flowHist,
            { color = colors.cyan, min = -maxAbs, max = maxAbs })
    end
...
local function renderApiTabDettaglio()
    ...
    -- Riga 5: nome prodotto + (i/N) + nav buttons inline
    p_lat4:button("api_prev", 3, 5, 4, 1, " < ", colors.blue, colors.white)
    p_lat4:text(9, 5, shortName(prod.id), P.fg_title, P.bg)
    p_lat4:text(W - 12, 5, ("(%d/%d)"):format(state.api_prod_idx, #products), P.fg_dim, P.bg)
    p_lat4:button("api_next", W - 6, 5, 4, 1, " > ", colors.blue, colors.white)

    p_lat4:text(3, 7,  "STORED:",  P.fg_label, P.bg)
    p_lat4:text(13, 7, fmt(s.count) .. " units", P.fg_value, P.bg)
    p_lat4:text(3, 8,  "RATE  :",  P.fg_label, P.bg)
    p_lat4:text(13, 8, fmtRate(s.rate_30m), (s.rate_30m >= 0) and P.fg_ok or P.fg_crit, P.bg)
    ...
    p_lat4:text(3, 11, "HISTORY 30M:", P.fg_dim, P.bg)
    local hist = bees.getHistory(prod.id, 60)
    if #hist >= 2 then
        local pts = {}
        for _, sm in ipairs(hist) do pts[#pts + 1] = sm.c end
        p_lat4:sparklineMulti(3, 12, W - 4, 2, pts, { color = colors.cyan })
    else
...
local function renderApiTabLista()
    ...
    p_lat4:text(18, 6, "STORED",   P.fg_label, P.bg)
    p_lat4:text(27, 6, "RATE",     P.fg_label, P.bg)
    p_lat4:text(35, 6, "TR",       P.fg_label, P.bg)
    p_lat4:fill(3, 7, W - 4, 1, colors.gray)

    local all = bees.snapshotAll()
    -- Mostra max 6 prodotti
    for i = 1, math.min(6, #all) do
        local s = all[i]
        local row = 7 + i
        if row >= 14 then break end
        p_lat4:text(3,  row, shorten(s.label, 12), P.fg_value, P.bg)
...
local function renderApiTabProiezione()
    ...
    p_lat4:text(3, 6, "IN:", P.fg_label, P.bg)
    p_lat4:fill(7, 6, 6, 1, colors.gray)
    p_lat4:text(9, 6, ("%4d"):format(state.api_proj_hours), P.fg_value, colors.gray)
    p_lat4:text(14, 6, "h", P.fg_label, P.bg)
    p_lat4:button("api_h_minus", 18, 6, 5, 1, " - ", colors.red,   colors.white)
    p_lat4:button("api_h_plus",  25, 6, 5, 1, " + ", colors.green, colors.white)

    p_lat4:fill(3, 7, W - 4, 1, colors.gray)
    p_lat4:text(3,  7,  "PRODUCT",  P.fg_value, colors.gray)
    p_lat4:text(18, 7,  "+" .. state.api_proj_hours .. "h", P.fg_value, colors.gray)
    p_lat4:text(28, 7,  "DELTA",    P.fg_value, colors.gray)
    ...
    for i, prod in ipairs(bees.getProducts()) do
        ...
        p_lat4:text(28, row, ((d >= 0) and "+" or "") .. fmt(d), dcol, P.bg)
    end
end

local function renderApiTabGrafici()
    local W, H = p_lat4:size()
    p_lat4:text(3, 5, " CHARTS 30M ", P.fg_title, P.bg)

    local products = bees.getProducts()
    local rowY = 6
    for _, prod in ipairs(products) do
        if rowY >= 14 then break end
        local s = bees.snapshot(prod.id)
        local hist = bees.getHistory(prod.id, 60)
        local pts = {}
        for _, sm in ipairs(hist) do pts[#pts + 1] = sm.c end

        p_lat4:text(3, rowY, shorten(shortName(prod.id), 9), P.fg_label, P.bg)
        if #pts >= 2 then
            p_lat4:sparklineMulti(13, rowY, W - 22, 1, pts, { color = colors.cyan })
        else
            p_lat4:text(13, rowY, "no data", P.fg_dim, P.bg)
        end
        local rateStr = fmtRate(s.rate_30m)
        local rcol = (s.rate_30m >= 0) and P.fg_ok or P.fg_crit
        p_lat4:text(W - 9, rowY, rateStr, rcol, P.bg)
...
local function renderApi()
    ...
    -- Header riga 3
    p_lat4:text(3, 3, "APIARY MONITOR", P.fg_title, P.bg)
    local ts = textutils.formatTime(os.time("local"), true)
    p_lat4:text(W - #ts - 2, 3, ts, P.fg_dim, P.bg)
    p_lat4:divider(1, 4, W, P.border)
    ...
    -- Tab buttons in basso (riga 14-15)
    ...
    local totalChars = 0
    for _, t in ipairs(tabs) do totalChars = totalChars + #t.label + 2 end
    local gap = math.max(1, math.floor((W - 4 - totalChars) / (#tabs - 1)))
    local x = 3
...
print("")
logger.info("Task paralleli avviati (v10)")
